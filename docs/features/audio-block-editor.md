...
## Validation feedback correction (2026-09-03)

A newly opened add form has three intentional validation blockers: **Name** is required, **ID** is required (and remains empty until a name is entered or it is edited directly), and at least one **Audio file** is required. The validator correctly assigned those errors to `name`, `id`, and `files`, but the UI initially hid untouched field messages. Since **Add block** is disabled while invalid, the user could not submit the form to reveal them and the footer could report three issues without identifying the fields.

The editor now keeps all current validation messages visible. It highlights the owning setting, file row, or files section and associates inputs using `aria-invalid` and `aria-describedby`. Per-file errors sit on the file row outside collapsed region details. Source-level hydration/parser and save-conflict messages now have a dedicated visible target in the sticky footer; previously they contributed to the issue count without any message element.

Follow-up testing found that an invalid source block's three missing-required parser messages were also copied into immutable hydration issues. Consequently, filling the form cleared its live field errors but immediately reintroduced the original source errors. Missing ID, name, and file parser messages are now considered editable and are not retained as unsafe source issues; unknown, duplicated, malformed, or unrepresentable syntax continues to block rewriting.

The validation and save rules have not changed. Focused tests verify both the exact three errors on an empty draft and their transition to no errors after a hydrated form is completed. On 2026-09-03, all 67 tests, ESLint, the TypeScript check, production build, emoji-data freshness check, and `git diff --check` passed. Manual Obsidian visual and assistive-technology checks remain advisable.
...
- Added global authoring defaults for type, loop, random order, autoplay, playlist end action, fade in, and initial volume. Default fade out, playlist crossfade, and volume automation are runtime parser defaults inherited dynamically when their block fields are omitted. Saved settings are normalized so absent/invalid new fields migrate to safe defaults.

### Global fade-out runtime inheritance (2026-09-03)

- Root cause: **Default fade out** was treated only as a value to copy into newly authored source. The parser assigned `fadeOutDuration: 0` whenever `fadeout` was omitted, so the inline and sidebar controls correctly interpreted the parsed definition as an immediate stop even when the global setting was non-zero.
- Fixed behavior: parsing now applies `defaultAudioBlockFadeOut` when `fadeout` is absent. The existing playback controls consequently show **Fade out** and call `fadeOutAndStop()` with the effective duration. A positive explicit `fadeout` overrides the default, while `fadeout: 0` explicitly retains immediate **Stop** behavior.
- **Play fade duration** still controls ordinary play, pause, and resume transitions. **Crossfade duration** still controls inter-block transitions. Neither setting supplies the Stop-button fade.
- The editor leaves the optional Fade out field blank for inherited behavior rather than serializing the current global value. Because parsed block definitions are created when Markdown renders, re-render an already-open block after changing the global default.
- Regression tests cover a single-file block containing only `id`, `name`, and `file`, positive per-block overrides, explicit zero, and fixed zero behavior when no plugin defaults are supplied. Verification: 68 tests, lint, TypeScript checking, production build, and `git diff --check` pass.
...
- Replaced vertically stacked folder sections with deselectable folder-filter tabs styled like the block Type selector. Tabs show only each folder's final path segment while retaining the full normalized folder path as the filter key, accessible label, and tooltip. Selecting the active tab again returns to one combined list; a horizontally scrollable tab rail keeps many folders usable.
- The combined and filtered lists sort alphabetically by filename in ascending order by default. An adjacent labeled icon button toggles ascending/descending order without changing selected files.
- Visual follow-up: inactive folder tabs use Obsidian's neutral secondary background, border, and muted-text variables for dependable contrast. The active tab uses `--interactive-accent` with `--text-on-accent`, while hover uses the theme's normal text and hover background. The sort control is explicitly neutral instead of accent/CTA-colored and shows both a stable arrow icon and its current `ASC`/`DESC` state; its accessible label also announces the current direction and available action.
...
- File-picker selectors now communicate cardinality through their native control type: replacement mode (`multiple: false`) uses one named radio group, while add-files mode and the default configuration use independent checkboxes. Selection state persists across folder filters, sorting, search, and single-file rerenders.
- File-picker rows now include an independent Play/Stop preview button. Hover or keyboard focus reveals Play on pointer devices; touch layouts keep the control visible. Only one picker-local `Audio` resource exists at a time, and it is stopped and released when another preview starts, filtering hides the active row, selection completes, the modal closes, playback ends, or playback fails. The selection label remains separate, preserving checkbox/radio behavior and native keyboard controls.
